quant_opt_harmonys <- function(seurat_obj) {
  pct <- seurat_obj[["harmony"]]@stdev / sum(seurat_obj[["harmony"]]@stdev) * 100 # Determine percent of variation associated with each PC
  ...
  p <-ggplot(plot_df, aes(cumu, pct, label = rank, color = rank > dims)) +
    ...
    ElbowPlot(seurat_obj,ndims = 30,reduction = "harmony") # Elbow plot to visualize
  return(p)
}
